...
dd if=$bootsect bs=512 count=1 of=$IMAGE  2>&1 >/dev/null   #del 'status=none' by tinynull

# Write setup(4 * 512bytes, four sectors)
[ ! -f "$setup" ] && echo "Error: No setup binary file there" && exit -1
dd if=$setup seek=1 bs=512 count=4 of=$IMAGE 2>&1 >/dev/null   #del 'status=none' by tinynull
...
dd if=$kernel seek=5 bs=512 of=$IMAGE 2>&1 >/dev/null     #del 'status=none' by tinynull 
